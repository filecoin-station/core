...
export class Activity {
  ...
  async get () {
    return (await fs.readFile(this.paths.activity, 'utf-8'))
      .trim()
      .split('\n')
      .filter(Boolean)
  ...
  async * follow ({ signal, nLines = 10 } = {}) {
    const tail = new Tail(this.paths.activity, {
      nLines,
  ...
  createWriteStream (source) {
    ...
    addMetaStream
      ...
      .pipe(this.logs.createWriteStream(this.paths.activity))
    return addMetaStream
  }

  async maybeCreateActivityFile () {
    await maybeCreateFile(this.paths.activity)
  }
}
