...
async function start ({
  ...
  childProcess.on('exit', (code, signal) => {
    ...
    console.error(msg)
    activities.emit('activity', { type: 'info', message: msg })
  })
  ...
  try {
    ...
    activities.emit('activity', { type: 'error', message })
  }
}
...
function handleActivityLogs (text) {
  text
    ...
    .forEach(line => {
      ...
      if (m) {
        activities.emit('activity', { type: m[1].toLowerCase(), message: m[2] })
      }
    })
}
...
async function updateStats (apiUrl) {
  ...
  metrics.submit('bacalhau', { totalJobsCompleted, totalEarnings: '0' })
}
