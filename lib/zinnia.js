...
export async function install () {
  await Promise.all([
    ...
    // temporarily disabled to test CI
...
async function start ({
  ...
  const zinniadExe = getBinaryModuleExecutable({ module: 'runtime', executable: 'zinniad' })
  const modules = [
  ...
  childProcess.on('close', code => {
    logStream.write(`Zinnia Runtime closed all stdio with code ${code ?? '<no code>'}`)
