...
import { installModule, getModuleExecutable } from './modules.js'
...
const matchesModuleFilter = module =>
  SUBNET_FILTER === '' || module === SUBNET_FILTER
...
const updateAllSourceFiles = async ({
  ...
  const subnets = await Promise.all(
    Object
      .values(SUBNETS)
      .filter(({ subnet }) => matchesModuleFilter(subnet))
      .map(({ subnet, ipnsKey }) =>
        pRetry(
...
export async function run ({
  ...
  const zinniadExe = getModuleExecutable({ module: 'zinnia', executable: 'zinniad' })
