...
/**
 * @param {object} args
 * @param {string} args.secretStore
 * @param {string} args.passphrase
 * @returns {Promise<{publicKey: string, privateKey: string}>}
 */
export async function getStationId ({ secretStore, passphrase }) {
  assert.strictEqual(typeof secretStore, 'string', 'stateRoot must be a string')
  assert.strictEqual(typeof passphrase, 'string', 'passphrase must be a string')

  await fs.mkdir(secretStore, { recursive: true })
  const keystore = path.join(secretStore, 'station_id')
  ...
  try {
    ...
    if (err.code === 'ENOENT' && err.path === keystore) {
      ...
      return await generateKeypair(passphrase, keystore)
    } else {
      throw err
    }
  }
