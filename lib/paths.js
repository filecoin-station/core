...
export const getRootDirs = () => {
  switch (platform()) {
    case 'darwin': {
      const appId = 'app.filstation.core'
      return [
        CACHE_ROOT || join(homedir(), 'Library', 'Caches', appId),
        STATE_ROOT || join(homedir(), 'Library', 'Application Support', appId)
      ]
    }
    case 'win32': {
      ...
      return [
        CACHE_ROOT || join(TEMP, appName),
        STATE_ROOT || join(LOCALAPPDATA, appName)
      ]
    }
    case 'linux': {
      const appSlug = 'filecoin-station-core'
      return [
        CACHE_ROOT || join(XDG_CACHE_HOME, appSlug),
        STATE_ROOT || join(XDG_STATE_HOME, appSlug)
      ]
    }
    default:
